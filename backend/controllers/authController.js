...
console.log(validUser);
  ...
  const isValidPassword =await validUser.comparePassword(password , validUser.password)
...
const {password:pass , ...rest} = find_user._doc
  ...
  const genaretedPassword = Math.random().toString(36).split(-8) + Math.random().toString(36).split(-8);

  const newUser = new User({username:name.split(" ").join("").toLowerCase() , password:genaretedPassword , email:email , avatar:photo})
...
module.exports = { signup , signin  , google_Auth };
